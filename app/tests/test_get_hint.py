...
def test_get_hint_partial_match():
    secret = "1234"
    guess = "1243"
    assert get_hint(secret, guess) == [2, 2], "Two digits are correct and in the right position, two are correct but wrong position"
...
def test_get_hint_duplicate_digits():
    secret = "1122"
    guess = "2211"
    assert get_hint(secret, guess) == [0, 4], "All digits match but none are in the correct position"
