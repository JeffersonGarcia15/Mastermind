...
def test_start_game_invalid_difficulty(
    client, requests_mock, db_session, flask_login_fixture
):
...
def test_make_guess_win_medium(client, requests_mock, db_session, flask_login_fixture):
    ...
    response = client.post(
        "/api/v2/game/guess", json={"game_id": game_id, "guess": "1234"}
    )
    json_data = response.get_json()
...
def test_make_guess_win_hard(client, requests_mock, db_session, flask_login_fixture):
    ...
    # Simulate a winning guess
    response = client.post(
        "/api/v2/game/guess", json={"game_id": game_id, "guess": "012345"}
    )
    json_data = response.get_json()
...
def test_make_guess_lose_medium(client, requests_mock, db_session, flask_login_fixture):
    ...
    for i in range(10):
        response = client.post(
            "/api/v2/game/guess",
            json={"game_id": game_id, "guess": "5678"},  # Incorrect guess
        )
        json_data = response.get_json()
...
def test_make_guess_lose_hard(client, requests_mock, db_session, flask_login_fixture):
    ...
    for i in range(10):
        response = client.post(
            "/api/v2/game/guess",
            json={"game_id": game_id, "guess": "567801"},  # Incorrect guess
        )
        json_data = response.get_json()
        assert json_data["data"]["status"] == ("lose" if i == 9 else "ongoing")
        assert json_data["data"]["attempts_left"] == (9 - i)
...
def test_make_guess_after_win(client, requests_mock, db_session, flask_login_fixture):
    ...
    # Simulate a winning guess
    client.post("/api/v2/game/guess", json={"game_id": game_id, "guess": "1234"})
...
def test_make_guess_after_lose(client, requests_mock, db_session, flask_login_fixture):
    ...
    response = client.post(
        "/api/v2/game/guess", json={"game_id": game_id, "guess": "5678"}
    )
    json_data = response.get_json()
...
def test_make_guess_invalid_game_id(
    client, requests_mock, db_session, flask_login_fixture
):
    """Test making a guess with an invalid game_id."""
    response = client.post(
        "/api/v2/game/guess", json={"game_id": "invalid-game-id", "guess": "1234"}
    )
    json_data = response.get_json()
...
def test_make_guess_invalid_payload_missing_game_id(
    client, db_session, flask_login_fixture
):
    """Test making a guess with missing game_id."""
    response = client.post("/api/v2/game/guess", json={"guess": "1234"})
    json_data = response.get_json()
...
def test_make_guess_invalid_payload_missing_guess(
    client, db_session, flask_login_fixture
):
    """Test making a guess with missing guess."""
    response = client.post("/api/v2/game/guess", json={"game_id": "some-game-id"})
    json_data = response.get_json()
...
def test_make_guess_invalid_guess_length(
    client, requests_mock, db_session, flask_login_fixture
):
    ...
    response = client.post(
        "/api/v2/game/guess",
        json={"game_id": game_id, "guess": "123"},  # Should be 4 digits
    )
    json_data = response.get_json()
...
def test_make_guess_non_digit_guess(
    client, requests_mock, db_session, flask_login_fixture
):
    ...
    response = client.post(
        "/api/v2/game/guess", json={"game_id": game_id, "guess": "12a4"}
    )
    json_data = response.get_json()
...
def test_start_game_medium_fallback(
    client, requests_mock, db_session, flask_login_fixture
):
    ...
    with patch(
        "app.api.game_routes.generate_local_sequence", return_value="0\n1\n2\n3"
    ):
        response = client.post(
            "/api/v2/game/start",
            json={
...
def test_start_game_hard_fallback(
    client, requests_mock, db_session, flask_login_fixture
):
...
def test_make_guess_with_fallback_sequence_revealed_on_win(
    client, requests_mock, db_session, flask_login_fixture
):
    ...
    with patch(
        ...
        # Simulate a winning guess
        response = client.post(
            "/api/v2/game/guess", json={"game_id": game_id, "guess": "0123"}
        )
        json_data = response.get_json()
...
def test_make_guess_with_fallback_sequence_revealed_on_lose(
    client, requests_mock, db_session, flask_login_fixture
):
