...
def test_authenticate_authenticated_user(client, db_session, flask_login_fixture):
    ...
    response_login = client.post(
        "/api/v2/auth/login", json={"username": "testuser", "password": "password123"}
    )
...
def test_login_missing_fields(client, db_session, flask_login_fixture):
    ...
    """
    response = client.post(
        "/api/v2/auth/login", json={"username": "userwithoutpassword"}
    )
    ...
    response = client.post(
        "/api/v2/auth/login", json={"password": "passwordwithoutusername"}
    )
...
def test_login_invalid_credentials(client, db_session, flask_login_fixture):
    ...
    response = client.post(
        "/api/v2/auth/login",
        json={"username": "wronguser", "password": "validpassword"},
    )
...
def test_logout_success(client, db_session, flask_login_fixture):
    ...
    response_login = client.post(
        "/api/v2/auth/login",
        json={"username": "logoutuser", "password": "logoutpassword"},
    )
...
def test_signup_success(client, db_session, flask_login_fixture):
    ...
    """
    response = client.post(
        "/api/v2/auth/signup", json={"username": "newuser", "password": "newpassword"}
    )
...
def test_signup_missing_fields(client, db_session, flask_login_fixture):
    ...
    """
    response = client.post(
        "/api/v2/auth/signup", json={"username": "userwithoutpassword"}
    )
    ...
    response = client.post(
        "/api/v2/auth/signup", json={"password": "passwordwithoutusername"}
    )
